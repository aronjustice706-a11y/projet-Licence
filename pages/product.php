...
$aaa = "<select class='form-control' name='category' required>
        <option disabled selected hidden>Sélectionner la catégorie</option>";
  while ($row = mysqli_fetch_assoc($result)) {
...
$sup = "<select class='form-control' name='supplier' required>
        <option disabled selected hidden>Sélectionner le fournisseur</option>";
...
?>
            ...
            <div class="card-body">
              <div class="table-responsive">
                <form method="post" action="pro_del_multi.php" id="multiDeleteForm" style="display:inline;">
                  <button type="submit" class="btn btn-danger mb-2 mr-2" onclick="return confirm('Supprimer les produits sélectionnés ?');">Supprimer la sélection</button>
    ...
    $query = 'SELECT PRODUCT_ID, PRODUCT_CODE, NAME, PRICE, CNAME, DATE_STOCK_IN FROM product p join category c on p.CATEGORY_ID=c.CATEGORY_ID GROUP BY PRODUCT_CODE';
        $result = mysqli_query($db, $query) or die (mysqli_error($db));
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '<tr>';
                echo '<td><input type="checkbox" name="selected_ids[]" value="'.$row['PRODUCT_ID'].'"></td>';
                echo '<td>'. $row['PRODUCT_CODE'].'</td>';
                echo '<td>'. $row['NAME'].'</td>';
                echo '<td>'. $row['PRICE'].' XAF</td>';
                echo '<td>'. $row['CNAME'].'</td>';
                      echo '<td align="right"> <div class="btn-group">
                            ...
                            <ul class="dropdown-menu text-center" role="menu">
                                ...
                                <li>
                                  <a type="button" class="btn btn-danger bg-gradient-danger btn-block" style="border-radius: 0px;" href="pro_del.php?id='.$row['PRODUCT_ID'].'&type=product" onclick="return confirm(&quot;Voulez-vous vraiment supprimer ce produit ?&quot;);">
